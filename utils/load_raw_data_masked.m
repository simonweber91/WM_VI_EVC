...
spm_mask_hdr = spm_vol(fullfile(p.base_dir, 'Nifti', ['sub-' sub_str], 'rois', 'mask.nii'));
spm_mask = spm_read_vols(spm_mask_hdr);
...
if isfield(p.psvr, 'roi') && ~isempty(p.psvr.roi)
    ...
    roi_dir = fullfile(p.base_dir, 'Nifti', ['sub-' sub_str], 'rois');
...
for i_ses = 1:p.n_session
    ...
    vols = dir(fullfile(p.base_dir, 'Nifti', ['sub-' sub_str], ['ses-' ses_str], 'func', ['rsub*' p.img.filter '*.nii']));
