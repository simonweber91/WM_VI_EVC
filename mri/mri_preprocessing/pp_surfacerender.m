...
if n_ses > 1
    ...
    check_file = dir(fullfile(base_dir, 'Nifti', ['sub-' sub_str], 'ses-01', 'anat', '*.surf.gii'));
    tissue = cellstr(spm_select('FPList', fullfile(base_dir, 'Nifti', ['sub-' sub_str], 'ses-01', 'anat'), ['^c.*rsub.*.nii']));
elseif n_ses == 1
    % Check if output images already exist
    check_file = dir(fullfile(base_dir, 'Nifti', ['sub-' sub_str], 'anat', '*.surf.gii'));
    tissue = cellstr(spm_select('FPList', fullfile(base_dir, 'Nifti', ['sub-' sub_str], 'anat'), ['^c.*rsub.*.nii']));
end
...
if isempty(check_file)
    ...
    if n_ses > 1
        render = dir(fullfile(base_dir, 'Nifti', ['sub-' sub_str], 'ses-01', 'anat', '*.surf.gii'));
    elseif n_ses == 1
        render = dir(fullfile(base_dir, 'Nifti', ['sub-' sub_str], 'anat', '*.surf.gii'));
    end
