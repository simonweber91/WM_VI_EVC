function [Y, mask] = spmReadVolsMasked(V, mask, output)
...
% output: amount of output printed to command window. []/0: no output, 1:
%       minimal output, 2: regular output
...
% 
% Output option added by Simon Weber, 09.02.2021
...
for vi = 1 : nVols
    ...
    % progress report
    if ~exist('output','var') || isempty(output) || output == 0 || output > 2
        % no output
    elseif output == 1      % minimal output
        if (vi == nVols)
            fprintf('  %d of %d volumes loaded\n', vi, nVols)
        end
    elseif output == 2      % regular output
