    ...
    for i_voxel = first_voxel:numel(p.psvr.voxel)
        ...
        for i_fwhm = first_fwhm:numel(p.psvr.fwhm)
            ...
            % parfor i_tr = 1:p.psvr.n_tr
                ...
                % Assign predictions
                predictions.sin_pred{i_voxel, i_tr, i_fwhm} = sin_pred;
                predictions.cos_pred{i_voxel, i_tr, i_fwhm} = cos_pred;
                predictions.ang_pred{i_voxel, i_tr, i_fwhm} = ang_pred;
                % Calculate balanced feature-continuous accuracy
                results.bfca(i_voxel, i_tr, i_fwhm) = bal_norm_circ_resp_dev(ang_pred, labels_rad, 'trapz') .* 100;
