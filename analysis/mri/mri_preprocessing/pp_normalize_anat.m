...
if n_ses > 1
    ...
    check_file = dir(fullfile(base_dir, 'Nifti', ['sub-' sub_str], 'ses-01', 'anat', ['wmrsub*']));
    deform_field = cellstr(spm_select('FPList', fullfile(base_dir, 'Nifti', ['sub-' sub_str], 'ses-01', 'anat'), '^y_rsub.*.nii'));
    files = cellstr(spm_select('ExtFPList', fullfile(base_dir, 'Nifti', ['sub-' sub_str], 'ses-01', 'anat'), '^mrsub.*T1w.nii'));
elseif n_ses == 1
    % Check if output images already exist
    check_file = dir(fullfile(base_dir, 'Nifti', ['sub-' sub_str], 'anat', ['wmrsub*']));
    deform_field = cellstr(spm_select('FPList', fullfile(base_dir, 'Nifti', ['sub-' sub_str], 'anat'), '^y_rsub.*.nii'));
    files = cellstr(spm_select('ExtFPList', fullfile(base_dir, 'Nifti', ['sub-' sub_str], 'anat'), '^mrsub.*T1w.nii'));
end
