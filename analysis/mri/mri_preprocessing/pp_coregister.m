...
if n_ses > 1
    ...
    check_file = dir(fullfile(base_dir, 'Nifti', ['sub-' sub_str], 'ses-01', 'anat', ['rsub*']));
    reference = cellstr(spm_select('FPList', fullfile(base_dir, 'Nifti', ['sub-' sub_str], 'ses-01', 'func'), '^meansub.*.nii'));
    source = cellstr(spm_select('FPList', fullfile(base_dir, 'Nifti', ['sub-' sub_str], 'ses-01', 'anat'), '^sub.*T1w.nii'));
elseif n_ses == 1
    % Check if output images already exist
    check_file = dir(fullfile(base_dir, 'Nifti', ['sub-' sub_str], 'anat', ['rsub*']));
    reference = cellstr(spm_select('FPList', fullfile(base_dir, 'Nifti', ['sub-' sub_str], 'func'), '^meansub.*.nii'));
    source = cellstr(spm_select('FPList', fullfile(base_dir, 'Nifti', ['sub-' sub_str], 'anat'), '^sub.*T1w.nii'));
end
