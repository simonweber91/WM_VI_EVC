...
bhvr_file = fullfile(out_dir, 'bhvr.mat');

if exist(bhvr_file, 'file') && ~p.OVERWRITE
    load(bhvr_file, 'bhvr.mat')
    return;
end
...
save(bhvr_file, 'bhvr', 'p');
catch
    warning('Behavioral data could not be analysed, likely because of missing files.')
end