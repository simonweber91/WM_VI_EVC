...
if n_ses > 1
    for i_ses = 1:n_ses
        ...
        if isempty(dir(fullfile(data_dir, 'Nifti', ['sub-' sub_str], ['ses-' ses_str], 'func', ['rp_sub*' filter '_all-runs*'])))

            % Get rp-file of current session and rename it
            rp_file = dir(fullfile(data_dir, 'Nifti', ['sub-' sub_str], ['ses-' ses_str], 'func', ['rp_sub*' filter '*']));
...
elseif n_ses == 1
    ...
    % Check if rp-files have already bee partitioned
    if isempty(dir(fullfile(data_dir, 'Nifti', ['sub-' sub_str], 'func', ['rp_sub*' filter '_all-runs*'])))

        % Get rp-file of current session and rename it
        rp_file = dir(fullfile(data_dir, 'Nifti', ['sub-' sub_str], 'func', ['rp_sub*' filter '*']));
        all_name = strrep(rp_file.name,'run-01','all-runs');
        movefile(fullfile(rp_file.folder, rp_file.name), fullfile(rp_file.folder, all_name));
