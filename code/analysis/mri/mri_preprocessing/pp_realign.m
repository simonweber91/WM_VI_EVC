...
if n_ses > 1
    ...
    check_file = dir(fullfile(data_dir, 'Nifti', ['sub-' sub_str], 'ses-01', 'func', ['meansub*' filter '*']));
    for i_ses = 1:n_ses
...
elseif n_ses == 1
    % Check if output images already exist
    check_file = dir(fullfile(data_dir, 'Nifti', ['sub-' sub_str], 'func', ['meansub*' filter '*']));
    for i_run = 1:n_run
        run_str = num2str(i_run,'%02i');
        list = spm_select('ExtFPList', fullfile(data_dir, 'Nifti', ['sub-' sub_str], 'func'), ['^sub.*' filter '.*run-' run_str]);
        files{end+1} = cellstr(list);
    end
end
...
if ~isempty(check_file) && p.OVERWRITE == 1
    ...
    for i_ses = 1:n_ses
        ses_str = num2str(i_ses,'%02i');
        delete(fullfile(data_dir, 'Nifti', ['sub-' sub_str], ['ses-' ses_str], 'func', ['rsub*' filter '*']));
        delete(fullfile(data_dir, 'Nifti', ['sub-' sub_str], ['ses-' ses_str], 'func', ['rp_sub*' filter '*']));
    end
    check_file = [];
end
