...
if n_ses > 1
    ...
    check_file = dir(fullfile(data_dir, 'Nifti', ['sub-' sub_str], 'ses-01', 'func', ['warsub*' filter '*']));
    ...
    for i_ses = 1:n_ses
         ...
         list = spm_select('ExtFPList', fullfile(data_dir, 'Nifti', ['sub-' sub_str], ['ses-' ses_str], 'func'),['^arsub.*' filter]);
         if ~isempty(list)
...
if ~isempty(check_file) && p.OVERWRITE == 1
    ...
    for i_ses = 1:n_ses
        ses_str = num2str(i_ses,'%02i');
        delete(fullfile(data_dir, 'Nifti', ['sub-' sub_str], ['ses-' ses_str], 'func', ['warsub*' filter '*']));
    end
    check_file = [];
end
