...
if n_ses > 1
    ...
    check_file = dir(fullfile(data_dir, 'Nifti', ['sub-' sub_str], 'ses-01', 'anat', ['c*rsub*']));
    files = cellstr(spm_select('FPList', fullfile(data_dir, 'Nifti', ['sub-' sub_str], 'ses-01', 'anat'), ['^rsub.*.nii']));
elseif n_ses == 1
    % Check if output images already exist
    check_file = dir(fullfile(data_dir, 'Nifti', ['sub-' sub_str], 'anat', ['c*rsub*']));
    files = cellstr(spm_select('FPList', fullfile(data_dir, 'Nifti', ['sub-' sub_str], 'anat'), ['^rsub.*.nii']));
end
