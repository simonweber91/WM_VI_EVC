...
if n_ses > 1
    ...
    check_file = dir(fullfile(data_dir, 'Nifti', ['sub-' sub_str], 'ses-01', 'func', ['swarsub*' filter '*']));
    for i_ses = 1:n_ses
         ses_str = num2str(i_ses,'%02i');
         list = spm_select('ExtFPList', fullfile(data_dir, 'Nifti', ['sub-' sub_str], ['ses-' ses_str], 'func'),['^warsub.*' filter]);
         if ~isempty(list)
...
elseif n_ses == 1
    % Check if output images already exist
    check_file = dir(fullfile(data_dir, 'Nifti', ['sub-' sub_str], 'func', ['swarsub*' filter '*']));
    list = spm_select('ExtFPList', fullfile(data_dir, 'Nifti', ['sub-' sub_str], 'func'),['^warsub.*' filter]);
    files{end+1} = cellstr(list);
end
...
if ~isempty(check_file) && p.OVERWRITE == 1
    ...
    for i_ses = 1:n_ses
        ses_str = num2str(i_ses,'%02i');
        delete(fullfile(data_dir, 'Nifti', ['sub-' sub_str], ['ses-' ses_str], 'func', ['swarsub*' filter '*']));
    end
    check_file = [];
end
