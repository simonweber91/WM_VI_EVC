...
if ~exist(fullfile(out_dir,'SPM.mat'),'file')
    ...
    if isempty(run_log)
        warning('Subject %d - no log files found, can''t perform 1st level specification.', sub_id)
        return;
    end
    ...
    for i_ses = 1:n_ses
        ...
        for i_run = 1:n_run
            ...
            rp_file = dir(fullfile(data_dir, 'Nifti', ['sub-' sub_str], ['ses-' ses_str], 'func', ['rp_sub*' filter '_run-' run_str '*']));
