...
data_dir                = p.dirs.data;                                       % base directory of study
...
for i_sub = 1:numel(subjects)
    ...
    for i_ses = 1:n_sessions
        ...
        dcm_dir = fullfile(data_dir, 'Dicom', ['sub-' sub_str], ['ses-' ses_str]);
        
        %%% Convert anatomical image %%%
        anat_in = cellstr(spm_select('FPList',dcm_dir,'dir',anat_filter));
        if ~isempty(anat_in{1})
        if numel(anat_in) > 2 
            warning(['Subject ' sub_str ' - more than 1 T1 image, skip.']);
        else
            anat_in = anat_in{end};
            anat_out = fullfile(data_dir, 'Nifti', ['sub-' sub_str], ['ses-' ses_str],'anat');
        ...
        func_out = fullfile(data_dir, 'Nifti', ['sub-' sub_str], ['ses-' ses_str],'func');
        
        fmap_dirs = cellstr(spm_select('FPList',dcm_dir,'dir',fmap_filter));
        fmpa_dirs = cellstr(spm_select('FPList',dcm_dir,'dir',fmpa_filter));
        fm_out = fullfile(data_dir, 'Nifti', ['sub-' sub_str], ['ses-' ses_str],'fmap');
        ...
        for i_func = 1:numel(func_dirs)
            ...
            warning(['No functional images found for subject ' sub_str '.']);
        end
        
        %%% Convert retinotopy %%%
        ret_in = cellstr(spm_select('FPList',dcm_dir,'dir',ret_filter));
        if ~isempty(ret_in{1})
        if numel(ret_in) > 2
            ...
            warning(['No retinotopy iamges found for subject ' sub_str '.']);
        end
    end
end
